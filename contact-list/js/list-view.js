...
function buildList() {
  const contactsWrapper = container.querySelector(`.contacts-list`);
  const contacts = JSON.parse(loadContacts());
  
  contactsWrapper.innerHTML = ``;
  
  contacts.forEach(contact => {
    addContact(contactsWrapper, buildContact(contact));
  });
}

function buildContact(contact) {
  return `<li data-email="${contact.email}" data-phone="${contact.phone}">
            <strong>${contact.name}</strong>
          </li>`;
}

function addContact(container, builtContact) {
  container.innerHTML += builtContact;
}
